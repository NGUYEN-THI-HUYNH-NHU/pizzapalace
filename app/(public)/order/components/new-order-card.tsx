...
export function NewOrderCard({ order, productCatalog, onOrderUpdated }: NewOrderCardProps) {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-4 shadow-xl">
                        <div className="mb-4 flex items-start justify-between gap-3">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-900">Chi tiết đơn hàng</h3>
                        ...
                        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <div className="rounded-xl bg-slate-50 p-3 md:col-span-1 xl:col-span-2">
                        ...
                        <div className="mt-4 rounded-xl border border-slate-100 bg-white p-3">
                            ...
                            <div className="space-y-3">
                                {order.orderItems.map((item, itemIndex) => {
                                    ...
                                    return (
                                        <div key={`${order.id}-${item.productId}-${item.sku}-${itemIndex}`} className="flex gap-3 rounded-xl border border-gray-100 p-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-start justify-between gap-3">
                                                    <div>
                                                        ...
                                                        {item.crustSize ? <p className="text-xs text-slate-500">Cỡ: {item.crustSize}</p> : null}
                                                        {item.crustName ? <p className="text-xs text-slate-500">Đế: {item.crustName}</p> : null}
                                                    </div>
                                                    <div>
                                                        <p className="text-end text-sm font-semibold text-slate-800">{currencyFormatter.format(lineTotal)}</p>
                                                        <p className="text-end text-sm text-slate-500">{item.quantity} x {currencyFormatter.format(item.price)}</p>
                                                    </div>
                                                </div>
                                                ...
                                                {item.selectedOptions.length > 0 ? (
                                                    <div className="mt-2 flex flex-col gap-2">
                                                        {item.selectedOptions.map((option, optionIndex) => (
                                                            <div key={`${item.productId}-${option.k}-${option.v}-${optionIndex}`} className="flex flex-col gap-2 rounded-sm bg-slate-100 px-2 py-1">
                                                                <span className="text-sm text-slate-500">{option.k}</span>
                                                                <span className="text-sm text-slate-900">{getSelectedOptionLabel(option, productCatalog)}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                ) : null}
            ...
            {isCancelOpen ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="w-full max-w-lg rounded-2xl bg-white p-4 shadow-xl">
                        ...
                        <p className="mt-1 text-sm text-slate-500">Chỉ có thể hủy đơn khi đơn đang ở trạng thái đã tiếp nhận.</p>
                        ...
                        <div className="mt-4 space-y-2 rounded-xl bg-slate-50 p-3 text-sm text-slate-700">
                            ...
                            <p><span className="font-medium">Khách hàng:</span> {order.customerName}</p>
                            <p><span className="font-medium">SĐT:</span> {order.customerPhone}</p>
                            <p><span className="font-medium">Tổng tiền:</span> {currencyFormatter.format(order.totalAmount)}</p>
                        </div>

                        <div className="mt-4 space-y-2">
                            <label htmlFor={`cancel-reason-${order.id}`} className="text-sm font-medium text-slate-700">
                                Lý do hủy đơn
                            </label>
                            <textarea
                        ...
                        <div className="mt-4 flex justify-end gap-2">
                            ...
                            <Button variant="destructive" disabled={isCancelling} onClick={onCancelOrder}>
