...
export default function OrderPage() {
    ...
    const handleSearch = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const payload = (await response.json().catch(() => ({}))) as OrderListResponse & { message?: string };
            ...
            setOrders([]);
            setSearchError(error instanceof Error ? error.message : 'Không thể tra cứu đơn hàng.');
        } finally {
    ...
    if (isBootstrapping) {
        return (
            <div className="w-full py-6">
                <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">Đang tải đơn hàng...</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full space-y-8 py-6">
                ...
                <>
                    <section className='flex justify-center'>
                        <div className="w-full lg:w-2xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                            <div className="text-slate-900 px-5 py-6 md:px-6">
                                <h2 className="mt-2 text-2xl font-bold">Tra cứu đơn hàng nhanh</h2>
                                <p className="mt-2 max-w-2xl text-sm text-slate-700">
                                    Nhập số điện thoại hoặc mã đơn hàng từ email xác nhận để xem trạng thái đơn.
                                </p>
                            </div>
                            ...
                            <form className="space-y-4 p-5 md:p-6" onSubmit={handleSearch}>
                                <div className="grid gap-4 md:grid-cols-2">
                                    ...
                                    <div>
                                        <label className="mb-2 block text-sm font-medium text-slate-700">Mã đơn hàng</label>
                                        <input
                                            type="text"
                                ...
                                {searchError && (
                                    <p className="rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{searchError}</p>
                                )}
                    ...
                    {!hasGuestResults ? (
                        <section className="rounded-2xl border border-dashed border-slate-300 bg-white p-5 shadow-sm">
                            <h3 className="text-lg font-bold text-slate-800">Cần gì để tra cứu?</h3>
                        ...
                        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                            <div className="mb-4 flex items-center justify-between">
                                <h2 className="text-xl font-bold text-slate-800">Kết quả tra cứu</h2>
                                <span className="text-sm text-slate-500">{guestMatchedOrders.length} đơn</span>
                            </div>
                ...
                <>
                    ...
                    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        {historyOrders.length === 0 ? (
                            <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">Bạn chưa có lịch sử đặt hàng.</p>
                        ) : (
            ...
            {isAuthenticated && !hasOrders ? (
                <section className="rounded-2xl border border-dashed border-slate-300 bg-white p-5 shadow-sm">
                    <p className="text-sm text-slate-600">Chưa có đơn hàng nào trong tài khoản này.</p>
                </section>
            ) : null}
        </div>
    );
